...
# when the script is invoked (prevents nested data/... paths).
...
def write_cfg(cfg_path: Path, runname: str, mode: str, ncores: int, rec_rel: str, lig_rel: str, ambig_rel: str):
    ...
    # execution mode ("local" or "batch")
    mode = "{mode}"
    ncores = {ncores}

    # molecules to be docked (data-relative paths)
    molecules =  [
        "{rec_rel}",
        "{lig_rel}"
        ]
    ...
    # ====================================================================
    # HADDOCK3 pipeline stages (executed in order)
    # ====================================================================

    # Topology generation — protonation and histidine states
    [topoaa]
    autohis = false
    [topoaa.mol1]
    nhisd = 0
    ...
    # Select top cluster representatives for final analysis
    [seletopclusts]
    top_models = 4
    # ====================================================================
    """
...
def _choose_runname(base_dir: Path, base_name: str, overwrite: bool) -> str:
    ...
    This is important for iterative benchmarking where users re-run with
    tweaked parameters.
    """
    candidate = base_name
    method_dir = base_dir
...
def haddock_pipeline(rec, lig, runname: str = None, opts: dict | None = None, **kwargs):
    ...
    ``run_haddock=True``) executes HADDOCK either locally or via a
    ...
    Returns:
        (run_dir, cfg_path, executed_cmd) — the run directory Path,
        config Path, and a human-readable string describing what was run.
    """
    ...
    # Determine runname: if not provided, base it on receptor_vs_ligand
    # and auto-increment to avoid overwriting previous runs.
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Config run_dir name.  Default: auto-increment (run1, run2, …) "
            "to avoid clobbering existing runs.  Set this explicitly when "
            "you want reproducible directory names for scripted benchmarks."
        ),
    )
    parser.add_argument(
        "--mode", default="local",
        help=(
            "HADDOCK execution mode (default: 'local').  Other values depend "
            "on your HADDOCK3 installation (e.g. 'batch' for HPC schedulers)."
        ),
    )
    parser.add_argument(
        "--ncores", type=int, default=4,
        help=(
            "CPU cores for HADDOCK (default: 4).  HADDOCK parallelises "
            "the rigid-body, flexible-refinement, and water-refinement "
            "stages across cores.  Match to your machine's physical cores."
        ),
    )
    parser.add_argument(
        "--ambig", default=None,
        help=(
            "Path to ambiguous restraints file (.tbl).  Restraints encode "
            "known or predicted interface residues and dramatically improve "
            "docking accuracy.  Omit for fully unbiased (ab-initio) docking, "
            "which is harder but needed when no experimental data exists."
        ),
    )
    parser.add_argument(
        "--run",
        action="store_true",
        help=(
            "Actually execute haddock3 after generating the config.  Without "
            "this flag the pipeline only stages files and writes the config, "
            "letting you review/edit before running manually.  Use --run for "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Overwrite an existing run directory with the same name.  "
            "Without this flag a FileExistsError is raised if the run_dir "
            "already exists, protecting previous results.  Use --overwrite "
            "when re-running after parameter changes."
        ),
    )
    parser.add_argument(
        "--container",
    ...
    try:
        receptor_path = resolve_input_path(args.receptor, search_root=args.input_dir)
    ...
    try:
        ligand_path = resolve_input_path(args.ligand, search_root=args.input_dir)
    except FileNotFoundError as e:
        print(f"ERROR: {e}", file=sys.stderr)
        print("Hint: use --input-dir to specify the directory containing "
              "your PDB files.", file=sys.stderr)
    ...
    try:
        ...
        print(f"ERROR: {e}", file=sys.stderr)
        print("Hint: use --overwrite to replace the existing run directory, "
